...
import { ExternalLink, Pencil, X, Plus, Trash2, Check } from 'lucide-react'
import DateFilter from './DateFilter'
import type { Topic, NewsItem, Optimization } from '@/types/insights'
...
export default function SignalFeed({ news, groupedNews, topics }: Props) {
  ...
  async function dismissOptimization(id: string) {
    await supabase.from('resource_optimizations').delete().eq('id', id)
    await fetchOptimizations()
  }
  ...
  async function save() {
    ...
    try {
      if (editState.topicName !== editState.originalName) {
        await supabase.from('topics').update({ name: editState.topicName }).eq('id', editState.topicId)
      }
      ...
      for (const id of originalIds) {
        if (!keepIds.has(id)) await supabase.from('resources').delete().eq('id', id)
      }

      for (const r of editState.resources) {
        if (r.id) {
          await supabase.from('resources').update({
            title: r.title, url: r.url || null, description: r.description || null,
          }).eq('id', r.id)
        } else {
          await supabase.from('resources').insert({
            topic_id: editState.topicId, title: r.title, url: r.url || null, description: r.description || null,
          })
        }
      }
  ...
  const adds = optimizations.filter(o => o.type === 'add')
  const removes = optimizations.filter(o => o.type === 'remove')
    ...
    <>
          ...
          <div className="flex items-center gap-2">
            ...
            {optimizations.length > 0 && (
              <button
                ...
                title={`${optimizations.length} optimization suggestion${optimizations.length > 1 ? 's' : ''}`}
              >
                ...
                <div className="space-y-4">
                  {items.map((item, i) => (
                    <div key={item.id} className={i > 0 ? 'border-t border-zinc-800/30 pt-4' : ''}>
                      {item.url ? (
                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="group flex items-start gap-1.5">
      ...
      {showOptimizations && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowOptimizations(false)} />
          <div className="relative z-10 bg-[#111113] border border-zinc-800 rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              {removes.length > 0 && (
                <div>
                  <p className="text-[9px] font-semibold tracking-[0.2em] text-zinc-600 uppercase mb-3">Remove</p>
                  <div className="space-y-2">
                    {removes.map(opt => (
                      <div key={opt.id} className="bg-zinc-900/60 border border-zinc-800/50 rounded-xl p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] font-medium text-zinc-300">{opt.title}</p>
                            <p className="text-[9px] text-zinc-600 uppercase tracking-wide mt-0.5">{opt.topics?.name}</p>
                            {opt.reason && <p className="text-[11px] text-zinc-500 mt-1.5 leading-relaxed">{opt.reason}</p>}
                          </div>
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              onClick={() => applyOptimization(opt)}
                              disabled={applyingId === opt.id}
                              className="flex items-center gap-1 text-[10px] text-red-400/80 hover:text-red-400 transition-colors disabled:opacity-40"
                            >
              ...
              {adds.length > 0 && (
                <div>
                  <p className="text-[9px] font-semibold tracking-[0.2em] text-zinc-600 uppercase mb-3">Add</p>
                  <div className="space-y-2">
                    {adds.map(opt => (
                      <div key={opt.id} className="bg-zinc-900/60 border border-zinc-800/50 rounded-xl p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[9px] text-zinc-600 uppercase tracking-wide mt-0.5">{opt.topics?.name}</p>
                            {opt.url && (
                              <a href={opt.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-zinc-600 hover:text-zinc-400 transition-colors flex items-center gap-1 mt-0.5">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              onClick={() => applyOptimization(opt)}
                              disabled={applyingId === opt.id}
                              className="flex items-center gap-1 text-[10px] text-green-500/70 hover:text-green-400 transition-colors disabled:opacity-40"
                            >
      ...
      {editState && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 bg-[#111113] border border-zinc-800 rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-800/60 shrink-0">
              <p className="text-[9px] font-semibold tracking-[0.22em] text-zinc-500 uppercase">Edit Topic</p>
              <button onClick={() => setEditState(null)} className="p-1 text-zinc-600 hover:text-zinc-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              <div>
                <label className="text-[9px] font-semibold tracking-[0.2em] text-zinc-600 uppercase block mb-2">Topic Name</label>
                <input
                  type="text"
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <label className="text-[9px] font-semibold tracking-[0.2em] text-zinc-600 uppercase">Resources</label>
                  <button onClick={addResource} className="flex items-center gap-1 text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors">
                    ...
                    {editState.resources.map((r, i) => (
                      <div key={i} className="bg-zinc-900/60 border border-zinc-800/50 rounded-xl p-4 space-y-3">
                        <div className="flex items-start gap-3">
                          <input
                            type="text" value={r.title} onChange={e => updateResource(i, 'title', e.target.value)} placeholder="Title"
                          ...
                          />
                          <button onClick={() => removeResource(i)} className="p-1 text-zinc-700 hover:text-red-400 transition-colors shrink-0 mt-0.5">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <input
                          type="url" value={r.url} onChange={e => updateResource(i, 'url', e.target.value)} placeholder="URL (optional)"
                        ...
                        />
                        <textarea
                          value={r.description} onChange={e => updateResource(i, 'description', e.target.value)} placeholder="Description (optional)" rows={2}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-zinc-800/60 shrink-0">
              <button onClick={() => setEditState(null)} className="text-[11px] text-zinc-500 hover:text-zinc-300 transition-colors px-4 py-2">
                Cancel
              </button>
              <button onClick={save} disabled={saving} className="text-[11px] font-medium bg-zinc-800 hover:bg-zinc-700 disabled:opacity-40 text-zinc-200 px-5 py-2 rounded-lg transition-colors">
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
